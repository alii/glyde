import gleam/json
import glyde/api/gateway_info as api
import glyde/identify_queue.{Deny, Request}
import glyde/internal/url
import glyde/model/gateway_info.{
...
pub fn the_endpoints_decode_their_own_responses_test() {
  assert answer(api.get_gateway_bot(), live_bot) == Ok(live())
  assert answer(api.get_gateway(), url_only)
    == Ok(GatewayInfo(
      url: "wss://gateway.discord.gg",
      dial_host: host("gateway.discord.gg"),
    ))
